<section class="section dashboard">
    <div class="row">
        <div class="col-md-12">
            <?php
                echo '<div class="alert alert-info alert-dismissible fade show" role="alert">';
                echo '  Berikut ini adalah halaman pengelolaan data pelanggan. Semua data pelanggan yang melakukan pendaftaran akan ditampilkan disini.';
                echo '  <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>';
                echo '</div>';
            ?>
        </div>
    </div>
    <div class="row">
        <div class="col-lg-4">
            <div class="card">
                <form action="javascript:void(0);" id="ProsesBatas">
                    <input type="hidden" name="page" id="page">
                    <div class="card-header">
                        <h4 class="card-title">
                            <i class="bi bi-search"></i> Cari Pelanggan
                        </h4>
                    </div>
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-12 mt-3">
                                <select name="batas" id="batas" class="form-control">
                                    <option value="5">5</option>
                                    <option selected value="10">10</option>
                                    <option value="25">25</option>
                                    <option value="50">50</option>
                                    <option value="100">100</option>
                                    <option value="250">250</option>
                                    <option value="500">500</option>
                                </select>
                                <small>Batas Tampilan Data</small>
                            </div>
                            <div class="col-md-12 mt-3">
                                <select name="keyword_by" id="keyword_by" class="form-control">
                                    <option value="">Pilih</option>
                                    <option value="nama">Nama Pelanggan</option>
                                    <option value="email">Email</option>
                                    <option value="kontak">Kontak</option>
                                </select>
                                <small>Mode Pencarian</small>
                            </div>
                            <div class="col-md-12 mt-3">
                                <input type="text" name="keyword" id="keyword" class="form-control">
                                <small>Pencarian</small>
                            </div>
                            <div class="col-md-12 mt-3">
                                <button type="submit" class="btn btn-md btn-dark btn-block btn-rounded" title="Mulai Pencarian">
                                    <i class="bi bi-search"></i> Cari
                                </button>
                            </div>
                        </div>
                    </div>
                </form>
            </div>
        </div>
        <div class="col-md-8">
            <div id="MenampilkanTabelPelanggan">
                
            </div>
        </div>
    </div>
</section>