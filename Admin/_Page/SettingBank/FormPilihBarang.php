<?php
    ...
    if($SessionAkses=="Admin"){
        ...
        if(empty($keyword)){
            $jml_data = mysqli_num_rows(mysqli_query($Conn, "SELECT*FROM barang WHERE id_mitra='$SessionIdMitra'"));
        }else{
            $jml_data = mysqli_num_rows(mysqli_query($Conn, "SELECT*FROM barang WHERE (id_mitra='$SessionIdMitra') AND (kode_barang like '%$keyword%' OR nama_barang like '%$keyword%' OR kategori_barang like '%$keyword%' OR satuan_barang like '%$keyword%')"));
        }
    }
?>
<script>
                    ...
                    <?php
                        if(empty($jml_data)){
                            ...
                            if($SessionAkses=="Admin"){
                                ...
                                if(empty($keyword)){
                                    $query = mysqli_query($Conn, "SELECT*FROM barang WHERE id_mitra='$SessionIdMitra' ORDER BY $OrderBy $ShortBy LIMIT $posisi, $batas");
                                }else{
                                    $query = mysqli_query($Conn, "SELECT*FROM barang WHERE (id_mitra='$SessionIdMitra') AND (kode_barang like '%$keyword%' OR nama_barang like '%$keyword%' OR kategori_barang like '%$keyword%' OR satuan_barang like '%$keyword%') ORDER BY $OrderBy $ShortBy LIMIT $posisi, $batas");
                                }
                            }
